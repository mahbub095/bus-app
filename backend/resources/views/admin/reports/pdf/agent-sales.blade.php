...
<head>
    <meta charset="utf-8">
    <title>Agent/Counter Sales Report</title>
    <style>
        ...
        table.data th { background: #1D4ED8; color: #fff; font-size: 9px; }
        table.data tr:nth-child(even) { background: #eff6ff; }
    </style>
</head>
<body>
    <h1>Agent / Counter Sales Report</h1>
    <div class="meta">
    ...
    <div class="summary">
        <table>
            <tr>
                <td class="label">Total Agents/Operators</td><td>{{ $summary['total_agents'] }}</td>
    ...
    <table class="data">
        <thead>
            <tr>
                <th>Operator / Agent</th><th>Routes Covered</th><th>Total Bookings</th>
                <th>Total Seats</th><th>Total Revenue (BDT)</th><th>AC Bookings</th><th>Non AC Bookings</th>
            </tr>
        </thead>
        <tbody>
            @forelse($rows as $row)
            ...
            @empty
                <tr><td colspan="7" style="text-align:center;">No agent sales data found for selected filters.</td></tr>
            @endforelse
        </tbody>
    </table>
</body>
