<div class="admin-sections-layout" style="grid-column: 1 / -1;">
                ...
                <tbody>
                    @forelse($promotions as $promo)
                        <tr>
                            ...
                            <td style="color: var(--gold); font-weight: bold;">BDT {{ number_format($promo->discount_amount) }}</td>
                            <td>{{ $promo->description }}</td>
                            <td>
                                <div class="action-btns">
                                    <button type="button" class="btn btn-secondary btn-sm"
                                        onclick="setCrudFormMode('promotion-form', {
                                            mode: 'edit',
                                            id: {{ $promo->id }},
                                    ...
                                    <form action="{{ route('admin.promotions.destroy', $promo->id) }}" method="POST" onsubmit="return confirm('Delete this coupon?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" style="text-align: center; padding: 30px; color: var(--text-muted)">No coupons found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="booking-form-sidebar">
        ...
        <form class="booking-form-fields" id="promotion-form" action="{{ route('admin.promotions.store') }}" method="POST">
            @csrf
            @method('POST')
            <input type="hidden" name="_edit_id" value="">
            <div class="input-group">
                <label>Coupon Code (e.g. SONYANEW)</label>
                <input type="text" name="code" class="coupon-input" placeholder="Code" required value="{{ old('code') }}">
            </div>
            <div class="input-group">
            ...
            <div class="input-group">
                ...
                <input type="text" name="description" class="coupon-input" placeholder="Voucher details" required value="{{ old('description') }}">
            </div>
            <button class="btn btn-primary" id="promotion-form-submit" type="submit" style="height: 42px; margin-top: 10px;">
                Generate Coupon
            </button>
