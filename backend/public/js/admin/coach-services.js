(function () {
    const stations              = window.CoachServices.stations;
    const searchUrl             = window.CoachServices.searchUrl;
    const cancelUrlTemplate     = window.CoachServices.cancelUrlTemplate;
    const toggleBlockUrlTemplate = window.CoachServices.toggleBlockUrlTemplate;
    const bookUrl               = window.CoachServices.bookUrl;
    ...
    function updateBookingExtras(schedule) {
        ...
        if (extrasEl) extrasEl.innerHTML = renderBookingExtrasHtml(schedule);
    }
    ...
    function generateDefaultGridForServices(layout, totalSeats) {
        ...
        if (layout === '2+2_last5') {
            ...
            let ri = 0;
            for (let r = 0; r < Math.ceil((totalSeats - 5) / 4); r++) {
                const L = letters[ri++];
                grid.push([{type:'seat',label:L+'1'},{type:'seat',label:L+'2'},{type:'aisle'},{type:'seat',label:L+'3'},{type:'seat',label:L+'4'}]);
            }
            const last = letters[ri];
            grid.push([1,2,3,4,5].map(n => ({type:'seat',label:last+n})));
            return grid;
        }
        if (layout === '1+2') {
            const grid = [[{type:'engine',label:'Engine'},{type:'aisle'},{type:'empty'},{type:'driver',label:'Driver'}]];
            let ri = 0;
            for (let r = 0; r < Math.ceil(totalSeats / 3); r++) {
                const L = letters[ri++];
                grid.push([{type:'seat',label:L+'1'},{type:'aisle'},{type:'seat',label:L+'2'},{type:'seat',label:L+'3'}]);
            }
            return grid;
        }
        if (layout === 'sleeper') {
            const lc = Math.ceil(totalSeats / 2), uc = totalSeats - lc;
            const lower = [[{type:'engine',label:'Engine'},{type:'aisle'},{type:'empty'},{type:'driver',label:'Driver'}]];
            let ri = 0;
            for (let r = 0; r < Math.max(0, Math.ceil((lc - 4) / 3)); r++) {
                const L = letters[ri++];
                lower.push([{type:'seat',label:'L-'+L+'1'},{type:'aisle'},{type:'seat',label:'L-'+L+'2'},{type:'seat',label:'L-'+L+'3'}]);
            }
            lower.push([1,2,3,4].map(n => ({type:'seat',label:'L-'+letters[ri]+n}))); ri++;
            const upper = [[{type:'empty'},{type:'aisle'},{type:'empty'},{type:'empty'}]];
            ri = 0;
            for (let r = 0; r < Math.max(0, Math.ceil((uc - 4) / 3)); r++) {
                const L = letters[ri++];
                upper.push([{type:'seat',label:'U-'+L+'1'},{type:'aisle'},{type:'seat',label:'U-'+L+'2'},{type:'seat',label:'U-'+L+'3'}]);
            }
            upper.push([1,2,3,4].map(n => ({type:'seat',label:'U-'+letters[ri]+n})));
            return { lower, upper };
        }
        // default 2+2
        const grid = [[{type:'engine',label:'Engine'},{type:'empty'},{type:'aisle'},{type:'empty'},{type:'driver',label:'Driver'}]];
        let ri = 0;
        for (let r = 0; r < Math.ceil(totalSeats / 4); r++) {
            const L = letters[ri++];
            grid.push([{type:'seat',label:L+'1'},{type:'seat',label:L+'2'},{type:'aisle'},{type:'seat',label:L+'3'},{type:'seat',label:L+'4'}]);
        }
        return grid;
    ...
    function renderSeatCell(schedule, seat, seatMap) {
        ...
        const cssParts  = isPicking ? ['selected'] : [`status-${status}`];
    ...
    function renderSeatMap(schedule) {
        ...
        if (typeof grid === 'string') { try { grid = JSON.parse(grid); } catch { grid = null; } }
        if (!grid || typeof grid !== 'object') grid = generateDefaultGridForServices(layout, totalSeats);

        const isSleeper = grid.lower !== undefined;
        const driverSvg = `<svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        function renderDeckHtml(deckGrid, hasDriver) {
            ...
            const engineHtml   = engineCell ? `<div class="seat status-engine" title="Engine cover" style="cursor:not-allowed;background-color:#374151;border-color:#1f2937;color:#9ca3af;font-size:9px;font-weight:bold;display:flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:8px;">ENG</div>` : `<div style="width:36px;"></div>`;
            const entranceHtml = `<span style="font-size:11px;color:var(--text-muted);font-weight:bold;">${hasDriver ? 'ENTRANCE' : 'UPPER DECK FRONT'}</span>`;
            const driverHtml   = driverCell ? `<div class="seat status-driver" title="Driver Seat" style="cursor:not-allowed;background-color:#10B981;border-color:#059669;color:#fff;display:flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:8px;">${driverSvg}</div>` : `<div style="width:36px;"></div>`;

            let html = `<div class="bus-blueprint">
        ...
        if (isSleeper) {
            return `<div class="sleeper-decks">
                <div><div class="deck-title">Lower Deck</div>${renderDeckHtml(grid.lower, true)}</div>
                <div><div class="deck-title">Upper Deck</div>${renderDeckHtml(grid.upper, false)}</div>
            </div>
        ...
        return `<div class="booking-form-sidebar">
            <div class="ticket-booking-panel">
                ...
                <select id="cs-dropping-point" class="ticket-field">
                    <option value="">Select dropping point</option>${droppingOpts}
                </select>
    ...
    function renderResults() {
        ...
        if (searchResults.length === 0) {
            listEl.innerHTML = `<div class="search-card text-center" style="padding:60px;text-align:center;">
                <h3>No Coaches Scheduled</h3>
                <p style="color:var(--text-secondary);margin-top:8px;">No scheduled buses match this criteria. Try another date or route.</p>
            </div>`;
            return;
        }
    ...
    function bindCoachListEvents() {
        ...
        listEl.addEventListener('change', e => {
            if (e.target.id === 'cs-boarding-point') {
                ...
                if (info) info.textContent = bp ? `Reporting: ${bp.reporting_time || '—'} · Departure: ${bp.departure_time || '—'}` : 'Select a boarding point';
            }
            if (e.target.id === 'cs-dropping-point') {
                ...
                if (info) info.textContent = dp ? `Estimated arrival: ${dp.arrival_time || '—'}` : 'Select a dropping point';
            }
            if (e.target.id === 'cs-booking-gender') adminGender = e.target.value;
        });
    }
    ...
    async function fetchCoachServices(silent = false) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (selectedSeatBooking && expandedScheduleId) {
                    const schedule = searchResults.find(s => s.id === expandedScheduleId);
                    selectedSeatBooking = schedule?.seat_bookings?.[selectedSeatBooking.seat]
                        ? { seat: selectedSeatBooking.seat, ...schedule.seat_bookings[selectedSeatBooking.seat] }
                        : null;
                }
        ...
        } catch (err) { console.error('Coach services fetch failed', err); }
        finally {
            isFetching = false;
            const btn = document.getElementById('cs-search-btn');
    ...
    async function handleBookingSubmit(schedule) {
        ...
        } catch { if (errorEl) { errorEl.style.display = 'block'; errorEl.textContent = 'Network error while submitting.'; } }
    }
